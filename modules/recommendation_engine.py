...
def show_recommendation():
    ...
    if "criteria_weights" not in st.session_state:
        st.warning("⚠️ Tentukan bobot kriteria terlebih dahulu.")
        return
    ...
    if (
        ...
        payoff_df = st.session_state["weighted_payoff_df"]
    else:
        payoff_df = st.session_state["payoff_df"]

    weights = list(st.session_state["criteria_weights"].values())
    ...
    best_ev = st.session_state.get("ev_result", ev.idxmax())
    best_eol = st.session_state.get("eol_result")
    best_utility = st.session_state.get("utility_result")
    best_mc = st.session_state.get("monte_carlo_result")
    ...
    st.subheader("📋 Recommendation by Method")
    ...
    vote_result.columns = ["Location", "Votes"]
    ...
    st.subheader("🗳️ Voting Result")
    st.dataframe(vote_result, use_container_width=True)
    ...
    ranking_df = ranking_df.sort_values(by="Votes", ascending=False)
    ...
    if len(ranking_df) >= 3:
        ...
        with col1:
            st.metric("🥇 Rank 1", ranking_df.iloc[0]["Location"])

        with col2:
            st.metric("🥈 Rank 2", ranking_df.iloc[1]["Location"])

        with col3:
            st.metric("🥉 Rank 3", ranking_df.iloc[2]["Location"])
    ...
    st.success(f"Lokasi terbaik adalah {final_choice}")
    ...
    st.info(f"""
    Lokasi {final_choice} dipilih sebagai alternatif terbaik karena memperoleh dukungan dari {total_votes} metode DSS.

    Tingkat konsensus keputusan mencapai {confidence}%.

    Recommendation Engine menggabungkan hasil dari:
    ...
    Semakin tinggi jumlah vote dan konsensus, semakin kuat rekomendasi lokasi tersebut.
    """)
    ...
    st.session_state["final_recommendation"] = final_choice
